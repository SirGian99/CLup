...
public class AccessControlInt {
    ...
    public Response confirmAccessRequest(String body) {
        ...
        return numberOfPeople > 0 && visitManager.confirmAccess(token, storeID, numberOfPeople) ?
                Response.status(Response.Status.OK).build() : Response.status(Response.Status.NOT_ACCEPTABLE).build();
    }
    ...
    public Response confirmExitRequest(String body) {
        ...
        String storeID = json.getString("storeID");
        int numberOfPeople = json.getInt("numberOfPeople");
        return numberOfPeople > 0 && visitManager.confirmExit(token, storeID, numberOfPeople) ?
                Response.status(Response.Status.OK).build() : Response.status(Response.Status.NOT_ACCEPTABLE).build();
    }
}